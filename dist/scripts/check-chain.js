...
async function main() {
    ...
    try {
        const c = new ethers_1.ethers.Contract(ARBSYS, abi, provider);
        const arbBn = await c.arbBlockNumber();
        console.log('ArbSys.arbBlockNumber():', arbBn.toString());
    }
    catch (e) {
        console.warn('ArbSys check skipped/failed:', e.message);
    }
    ...
    for (const [label, addr] of [['WETH', WETH], ['USDC(native)', USDCn], ['USDC.e', USDCe]]) {
