...
function envWss(name) { return (process.env[name] || "").trim(); }
...
function pickQuickNodeWss() {
    return envWss("QUICKNODE_ARB_WSS") || envWss("QNODE_ARB_WSS") || null;
}
// Helpers
const sleep = (ms) => new Promise(res => setTimeout(res, ms));
function formatEth(v) { try {
    return ethers_1.ethers.formatEther(v || 0n);
}
catch {
    return "0";
} }
...
async function startNativePending(tag, wssUrl, onHit, onHead) {
    const provider = new ethers_1.ethers.WebSocketProvider(wssUrl, 42161);
    let closed = false;
    const pendingHandler = (hash) => { onHit(hash); };
    provider.on("pending", pendingHandler);
    let headCount = 0;
    const headHandler = (bn) => {
        headCount++;
    ...
    const close = () => {
        ...
        try {
            provider.off("block", headHandler);
        }
        catch { }
        try {
            provider.destroy();
...
function startAlchemyFiltered(alchUrl, toAddresses, onHit) {
    const ws = new ws_1.default(alchUrl, { perMessageDeflate: false });
    let closed = false;
    let subId = null;
    let id = 0;
    const nextId = () => ++id;
...
(async function main() {
    ...
    if (nativeUrl) {
        ...
        closeFn = await startNativePending("native", nativeUrl, async (hash) => {
            try {
                const http = new ethers_1.ethers.JsonRpcProvider(process.env.ALCH_ARB_HTTP || process.env.QUICKNODE_ARB_HTTP || process.env.LLAMA_ARB_HTTP || process.env.INFURA_ARB_HTTP || "", { name: "arbitrum", chainId: 42161 });
                const tx = await http.getTransaction(hash);
                if (!tx || !tx.to || !tx.data)
                    return;
                const to = lower(tx.to);
                if (!WATCHED_ROUTERS.includes(to))
                    return;
    ...
    if (haveAlchemy()) {
        ...
        closeFn = startAlchemyFiltered(alchUrl, WATCHED_ROUTERS, async (res) => {
            try {
                if (typeof res === "string") {
                    // hashesOnly path
                    const hash = res;
                    const http = new ethers_1.ethers.JsonRpcProvider(process.env.ALCH_ARB_HTTP || "", { name: "arbitrum", chainId: 42161 });
                    const tx = await http.getTransaction(hash);
                    if (!tx || !tx.to || !tx.data)
                        return;
                    const to = lower(tx.to);
                ...
                if (!to || !WATCHED_ROUTERS.includes(to))
                    return;
                if (sig && !SIGS.has(sig))
                    return;
    ...
    if (qnUrl) {
        ...
        closeFn = await startNativePending("qn", qnUrl, async (hash) => {
            try {
                const http = new ethers_1.ethers.JsonRpcProvider(process.env.QUICKNODE_ARB_HTTP || "", { name: "arbitrum", chainId: 42161 });
                const tx = await http.getTransaction(hash);
                if (!tx || !tx.to || !tx.data)
                    return;
                const to = lower(tx.to);
