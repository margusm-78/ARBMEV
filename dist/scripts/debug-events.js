...
function getProvider() {
    ...
    if (!url)
        throw new Error("Set ARB_RPC_URL in .env");
...
async function main() {
    ...
    const pool = new ethers_1.ethers.Contract(poolAddr, MIN_ABI, provider);
    ...
    const swapTopic = ethers_1.ethers.id("Swap(address,address,int256,int256,uint160,uint128,int24)");
    const filter = { address: poolAddr, topics: [swapTopic] };
    const rawListener = (log) => {
