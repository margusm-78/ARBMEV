...
const provider = new ethers_1.ethers.JsonRpcProvider(process.env.ANVIL_URL || 'http://127.0.0.1:8545', { name: 'arbitrum', chainId: 42161 });
...
/* ---------- Quote helpers ---------- */
async function quoteV3Best(amountIn, targetStable) {
    const quoter = new ethers_1.ethers.Contract(QUOTER_V2, QUOTER_V2_ABI, provider);
    let best = null;
    // single-hop
    for (const fee of FEES) {
        try {
            ...
            if (!best || out > best.amountOut)
                best = { dex: 'V3', kind: 'single', fee, amountOut: out, tokenOut: targetStable };
        }
        catch { }
    }
    // two-hop via USDT
    for (const feeAB of FEES)
        for (const feeBC of FEES) {
            try {
                ...
                if (!best || out > best.amountOut)
                    best = { dex: 'V3', kind: 'multi', feeAB, feeBC, amountOut: out, tokenOut: targetStable };
            }
            catch { }
        }
    return best;
}
async function quoteV2(routerAddr, amountIn, path) {
    const r = new ethers_1.ethers.Contract(routerAddr, V2_ROUTER_ABI, provider);
    try {
...
async function quoteV2Best(amountIn) {
    const candidates = [
    ...
    for (const c of candidates) {
        ...
        if (out > 0n && (!best || out > best.amountOut))
            best = { router: c.router, path: c.path, label: c.label, amountOut: out };
    }
    return best;
}
/* ---------- Main ---------- */
async function main() {
    ...
    const wallet = new ethers_1.ethers.Wallet(normalizePk(process.env.PRIVATE_KEY), provider);
    ...
    const weth = new ethers_1.ethers.Contract(WETH, WETH9_ABI, wallet);
    const amountIn = ethers_1.ethers.parseEther(WETH_IN);
    ...
    if (!bestV3)
        bestV3 = await quoteV3Best(amountIn, USDC_E);
    ...
    if (bestV3 && (!bestV2 || bestV3.amountOut >= bestV2.amountOut)) {
        choice = bestV3.kind === 'single'
            ? { which: 'V3-single', fee: bestV3.fee, amountOut: bestV3.amountOut, tokenOut: bestV3.tokenOut }
            : { which: 'V3-multi', feeAB: bestV3.feeAB, feeBC: bestV3.feeBC, amountOut: bestV3.amountOut, tokenOut: bestV3.tokenOut };
    }
    else if (bestV2) {
        choice = { which: 'V2', router: bestV2.router, path: bestV2.path, label: bestV2.label, amountOut: bestV2.amountOut };
    }
    if (!choice)
        throw new Error('No viable route on V3 or V2 — try a newer fork block or increase WETH_IN.');
    const outDecimals = 6; // USDC/USDC.e
    const minOut = (choice.amountOut * BigInt(10000 - SLIPPAGE_BPS)) / 10000n;
    console.log(`Best route: ${choice.which === 'V2' ? choice.label : choice.which} | quoted ≈ ${(Number(choice.amountOut) / (10 ** outDecimals)).toFixed(2)} USDC(e), minOut=${minOut}`);
    ...
    if (choice.which.startsWith('V3')) {
        ...
        await (await weth.approve(V3_ROUTER, ethers_1.ethers.MaxUint256, { nonce: nonce++ })).wait();
    }
    else {
        const routerAddr = choice.router;
        console.log(`Approving V2 router to spend WETH: ${routerAddr}`);
        await (await weth.approve(routerAddr, ethers_1.ethers.MaxUint256, { nonce: nonce++ })).wait();
    }
    // 6) Execute the swap (explicit nonce)
    const deadline = BigInt(Math.floor(Date.now() / 1000) + 600);
    if (choice.which === 'V3-single') {
        const router = new ethers_1.ethers.Contract(V3_ROUTER, V3_ROUTER_ABI, wallet);
        const params = [WETH, choice.tokenOut, choice.fee, addr, deadline, amountIn, minOut, 0n];
        ...
        const router = new ethers_1.ethers.Contract(V3_ROUTER, V3_ROUTER_ABI, wallet);
        const path = encodePath2Hop(WETH, choice.feeAB, USDT, choice.feeBC, choice.tokenOut);
        ...
        const router = new ethers_1.ethers.Contract(choice.router, V2_ROUTER_ABI, wallet);
        console.log(`Swapping on V2 router: ${choice.router} ...`);
        await (await router.swapExactTokensForTokens(amountIn, minOut, choice.path, addr, Number(deadline), { nonce: nonce++ })).wait();
    }
    // 7) Approve V3 router for the resulting stable so later code can spend it (explicit nonce)
    const tokenOutFinal = (choice.which === 'V2') ? choice.path.slice(-1)[0] : choice.tokenOut;
    const stable = new ethers_1.ethers.Contract(tokenOutFinal, ERC20_ABI, wallet);
    const allowance = await stable.allowance(addr, V3_ROUTER);
    if (allowance < ethers_1.ethers.MaxUint256 / 2n) {
        console.log('Approving Uniswap V3 router for stable (MaxUint256)...');
        await (await stable.approve(V3_ROUTER, ethers_1.ethers.MaxUint256, { nonce: nonce++ })).wait();
    }
    const dec = 6;
    const bal = await (new ethers_1.ethers.Contract(tokenOutFinal, ERC20_ABI, provider)).balanceOf(addr);
