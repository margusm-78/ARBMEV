...
const PRIORITY = (process.env.RPC_PRIORITY || "alchemy,quicknode,llama,infura")
    .split(",")
...
function collectCandidates() {
    const out = [];
    const add = (tag, kind, name) => {
        const v = val(name);
        if (v)
            out.push({ tag, kind, url: v });
    };
    // Preferred names from earlier chats
    add("alchemy", "http", "ALCH_ARB_HTTP");
    add("alchemy", "ws", "ALCH_ARB_WSS");
    add("quicknode", "http", "QNODE_ARB_HTTP");
    add("quicknode", "ws", "QNODE_ARB_WSS");
    add("quicknode", "http", "QUICKNODE_ARB_HTTP");
    add("quicknode", "ws", "QUICKNODE_ARB_WSS");
    add("llama", "http", "LLAMA_ARB_HTTP");
    add("llama", "ws", "LLAMA_ARB_WSS");
    add("infura", "http", "INFURA_ARB_HTTP");
    add("infura", "ws", "INFURA_ARB_WSS");
    // Generic fallbacks (keep infura last if user sets ARB_RPC_URL)
    const genericHttp = val("ARB_RPC_URL");
    if (genericHttp)
        out.push({ tag: detectTag(genericHttp), kind: "http", url: genericHttp });
    const genericWs = val("ARB_WS_URL") || val("ARB_WSS_URL");
    if (genericWs)
        out.push({ tag: detectTag(genericWs), kind: "ws", url: genericWs });
    // Old names
    const b = val("ARB_RPC_URL_BACKUP");
    if (b)
        out.push({ tag: detectTag(b), kind: "http", url: b });
    return out;
}
function detectTag(url) {
    const u = url.toLowerCase();
    if (u.includes("alchemy.com"))
        return "alchemy";
    if (u.includes("quicknode") || u.includes("quiknode"))
        return "quicknode";
    if (u.includes("llamanodes") || u.includes("llama"))
        return "llama";
    if (u.includes("infura"))
        return "infura";
    return "other";
}
...
class ResilientProvider {
    states = [];
    window = new WindowCounter(300);
    current;
    ready = false;
    // Expose http provider for normal usage
    get provider() {
        if (!this.current?.http)
            throw new Error("RP not ready; call RP.ensureReady()");
    ...
    async ensureReady() {
        ...
        this.states = tags.map(tag => {
            ...
            const st = {
                tag,
                http: httpUrl ? new ethers_1.ethers.JsonRpcProvider(httpUrl, { name: "arbitrum", chainId: 42161 }) : undefined,
                ws: wsUrl ? new ethers_1.ethers.WebSocketProvider(wsUrl, 42161) : undefined,
                ok: false,
            };
            return st;
        });
        ...
        if (!this.current?.http)
            throw new Error("No healthy RPCs after probing.");
        this.ready = true;
    ...
    async withProvider(fn) {
        ...
        for (let attempt = 0; attempt < this.states.length; attempt++) {
            ...
            if (!st || tried.has(st))
                st = this.pickNext(tried);
            if (!st)
                throw new Error("RP: No provider eligible to use right now");
            tried.add(st);
            // Skip if circuit open
            if (st.failedUntil && Date.now() < st.failedUntil)
                continue;
            try {
    ...
    onNewHeads(onBlock, onError) {
        const ws = this.bestWs();
        if (ws) {
            const handler = (bn) => onBlock(bn);
            ws.on("block", handler);
            const off = () => { try {
    ...
    subscribeLogs(filter, onLog, onError) {
        const ws = this.bestWs();
        if (ws) {
            const handler = (log) => onLog(log);
            ws.on(filter, handler);
            const off = () => { try {
                ws.off(filter, handler);
            }
            catch { } };
            return off;
        }
            ...
            let stopped = false;
            let from = filter?.fromBlock ?? "latest";
            const poll = async () => {
                while (!stopped) {
                    try {
                        const logs = await this.withProvider(p => p.getLogs({ ...filter, fromBlock: from, toBlock: "latest" }));
    ...
    penaltyMs(e) {
        ...
        if (code === "SERVER_ERROR" && /rate limit|429/i.test(e?.message || ""))
            return 60_000;
...
exports.RP = new ResilientProvider();
