...
exports.DEXS = exports.TOKENS = exports.ADDR = exports.CFG = void 0;
exports.runProfitableMEVBot = runProfitableMEVBot;
exports.startContinuousMonitoring = startContinuousMonitoring;
exports.findCrossDEXOpportunities = findCrossDEXOpportunities;
exports.findTriangularOpportunity = findTriangularOpportunity;
exports.findMultiTokenOpportunities = findMultiTokenOpportunities;
exports.findFlashLoanOpportunity = findFlashLoanOpportunity;
exports.quoteUniV3 = quoteUniV3;
exports.quoteV2Router = quoteV2Router;
exports.quoteCurve = quoteCurve;
exports.quoteBalancer = quoteBalancer;
...
const CFG = {
    ...
    PROBE_NOTIONAL_A: ethers_1.ethers.parseUnits((process.env.PROBE_NOTIONAL_A ?? "0.01").trim(), 18),
    MIN_PROFIT_ARB: ethers_1.ethers.parseUnits((process.env.MIN_PROFIT_ARB ?? "0.002").trim(), 18),
    MAX_GAS_COST_ARB: ethers_1.ethers.parseUnits((process.env.MAX_GAS_COST_ARB ?? "0.001").trim(), 18),
    // Toggles
    ...
    WETH_BASE_AMOUNT: ethers_1.ethers.parseUnits((process.env.WETH_BASE_AMOUNT ?? "0.001").trim(), 18),
    USDC_BASE_AMOUNT: ethers_1.ethers.parseUnits((process.env.USDC_BASE_AMOUNT ?? "1.0").trim(), 6),
    USDT_BASE_AMOUNT: ethers_1.ethers.parseUnits((process.env.USDT_BASE_AMOUNT ?? "1.0").trim(), 6),
    // Scanning/monitoring
    ...
    // Safety
    MAX_TRADE_SIZE_ARB: ethers_1.ethers.parseUnits((process.env.MAX_TRADE_SIZE_ARB ?? "1.0").trim(), 18),
    DAILY_PROFIT_TARGET_ARB: ethers_1.ethers.parseUnits((process.env.DAILY_PROFIT_TARGET_ARB ?? "0.1").trim(), 18),
    DAILY_LOSS_LIMIT_ARB: ethers_1.ethers.parseUnits((process.env.DAILY_LOSS_LIMIT_ARB ?? "0.05").trim(), 18),
    // Logging & ranking
...
exports.DEXS = DEXS;
/* ------------------------------ Validation ------------------------------ */
async function validateContract(provider, address, name) {
    if (!address || !ethers_1.ethers.isAddress(address) || address === ethers_1.ethers.ZeroAddress) {
        if (CFG.LOG_ALL_QUOTES)
            console.log(`⚠️ Invalid ${name} address: ${address}`);
        return false;
    }
    try {
...
async function quoteUniV3(provider, tokenIn, tokenOut, fee, amountIn) {
    if (amountIn <= 0n)
        return 0n;
    try {
        const quoter = new ethers_1.ethers.Contract(ADDR.UNI_QUOTER, QUOTER_V2_ABI, provider);
        const res = await quoter.quoteExactInputSingle.staticCall({ tokenIn, tokenOut, amountIn, fee, sqrtPriceLimitX96: 0n });
        const out = res?.amountOut;
        return out ?? 0n;
    }
    catch (error) {
        if (CFG.LOG_ALL_QUOTES)
            console.log(`UniV3 quote failed (${tokenIn}->${tokenOut}, fee ${fee}):`, error);
        return 0n;
    }
}
...
async function quoteV2Router(provider, routerAddr, path, amountIn) {
    if (amountIn <= 0n)
        return 0n;
    if (!await validateContract(provider, routerAddr, "V2Router"))
        return 0n;
    try {
        const router = new ethers_1.ethers.Contract(routerAddr, V2_ROUTER_ABI, provider);
        const amounts = await router.getAmountsOut(amountIn, path);
        return (Array.isArray(amounts) && amounts.length > 1) ? amounts[amounts.length - 1] : 0n;
    }
    catch (error) {
        if (CFG.LOG_ALL_QUOTES)
            console.log(`V2Router quote failed (${routerAddr}):`, error);
        return 0n;
    }
}
/* ------------------------------ Curve (stable-only) ------------------------------ */
...
async function quoteCurve(provider, tokenIn, tokenOut, amountIn) {
    if (!CFG.ENABLE_CURVE || !ADDR.CURVE_ROUTER || amountIn <= 0n)
        return { pool: "", amountOut: 0n };
    try {
        const router = new ethers_1.ethers.Contract(ADDR.CURVE_ROUTER, CURVE_ROUTER_ABI, provider);
        const [pool, amountOut] = await router.get_best_rate(tokenIn, tokenOut, amountIn);
        return { pool, amountOut: amountOut };
    }
    catch (error) {
        if (CFG.LOG_ALL_QUOTES)
            console.log(`Curve quote failed:`, error);
        return { pool: "", amountOut: 0n };
    }
}
/* ------------------------------ Balancer (placeholder) ------------------------------ */
async function quoteBalancer(_provider, _tokenIn, _tokenOut, _amountIn) {
...
const dexPerf = {};
function bumpDexPerf(name, field) {
    dexPerf[name] = dexPerf[name] ?? { quotes: 0, wins: 0 };
    dexPerf[name][field]++;
}
async function getQuotesForPair(provider, tokenInAddr, tokenOutAddr, tokenInSymbol, tokenOutSymbol, amountIn) {
    if (amountIn <= 0n)
        return [];
    const quotes = [];
    ...
    if (CFG.LOG_ALL_QUOTES && quotes.length)
        console.log(`  ${pairName}: ${quotes.length} quotes`);
    return quotes;
}
/* ------------------------------ Cross-DEX multi-pair ------------------------------ */
async function findCrossDEXOpportunities(provider) {
    if (!CFG.ENABLE_CROSS_DEX)
    ...
    const pairsToProbe = [
        ...
        ...(CFG.ENABLE_DEEP_SCANNING
            ? CFG.PRIMARY_PAIRS.concat(CFG.SECONDARY_PAIRS)
                ...
                tokenIn: a,
                tokenOut: b,
                amount: a === "WETH" ? CFG.WETH_BASE_AMOUNT :
                    a === "USDC" ? CFG.USDC_BASE_AMOUNT :
                        a === "USDT" ? CFG.USDT_BASE_AMOUNT :
                            ethers_1.ethers.parseUnits("1", TOKENS[a].decimals),
            }))
            : []),
    ];
    let best = { profitable: false };
    for (const pair of pairsToProbe) {
        const ti = TOKENS[pair.tokenIn];
        const to = TOKENS[pair.tokenOut];
        if (!ti || !to)
            continue;
        ...
        console.log(`  Best buy: ${bestBuy.name} → ${ethers_1.ethers.formatUnits(bestBuy.amountOut, to.decimals)} ${to.symbol}`);
        ...
        if (profit > 0n && ti.symbol !== "ARB") {
            if (ti.symbol === "WETH")
                profitInArb = await quoteUniV3(provider, ADDR.WETH, ADDR.ARB, 3000, profit);
            else if (ti.symbol === "USDC")
                profitInArb = await quoteUniV3(provider, ADDR.USDC, ADDR.ARB, 3000, profit);
            else if (ti.symbol === "USDT")
                profitInArb = await quoteUniV3(provider, ADDR.USDT, ADDR.ARB, 3000, profit);
        }
        ...
        console.log(`  Δ ${ti.symbol}: ${ethers_1.ethers.formatUnits(profit, ti.decimals)}`);
        console.log(`  ≈ ARB: ${ethers_1.ethers.formatUnits(profitInArb, 18)} | Net: ${ethers_1.ethers.formatUnits(netArb, 18)} ARB`);
...
async function findTriangularOpportunity(provider) {
    ...
    for (const path of paths) {
        if (!path.tokens.every(t => TOKENS[t]))
            continue;
        const startTok = TOKENS[path.tokens[0]];
        const startAmt = startTok.symbol === "ARB" ? CFG.PROBE_NOTIONAL_A :
            startTok.symbol === "WETH" ? CFG.WETH_BASE_AMOUNT :
                startTok.symbol === "USDC" ? CFG.USDC_BASE_AMOUNT :
                    startTok.symbol === "USDT" ? CFG.USDT_BASE_AMOUNT :
                        ethers_1.ethers.parseUnits("1", startTok.decimals);
        console.log(`\nPath: ${path.name} with ${ethers_1.ethers.formatUnits(startAmt, startTok.decimals)} ${startTok.symbol}`);
        let amt = startAmt;
        let ok = true;
        const steps = [];
        for (let i = 0; i < path.tokens.length - 1; i++) {
            ...
            if (out === 0n) {
                ok = false;
                if (CFG.LOG_ALL_QUOTES)
                    console.log(`  Fail ${from.symbol}->${to.symbol} (fee ${fee})`);
                break;
            }
        ...
        if (startTok.symbol !== "ARB" && gross > 0n) {
            if (startTok.symbol === "WETH")
                profitArb = await quoteUniV3(provider, ADDR.WETH, ADDR.ARB, 3000, gross);
            else if (startTok.symbol === "USDC")
                profitArb = await quoteUniV3(provider, ADDR.USDC, ADDR.ARB, 3000, gross);
            else if (startTok.symbol === "USDT")
                profitArb = await quoteUniV3(provider, ADDR.USDT, ADDR.ARB, 3000, gross);
        }
        const netArb = profitArb - CFG.MAX_GAS_COST_ARB;
        const bps = gross > 0n ? Number(gross * 10000n / startAmt) : 0;
        console.log(`  Result: ${ethers_1.ethers.formatUnits(startAmt, startTok.decimals)} → ${ethers_1.ethers.formatUnits(finalAmt, startTok.decimals)} ${startTok.symbol}`);
        console.log(`  Gross:  ${ethers_1.ethers.formatUnits(gross, startTok.decimals)} ${startTok.symbol} (${bps} bps)`);
        console.log(`  Net(ARB): ${ethers_1.ethers.formatUnits(netArb, 18)} ARB`);
        if (netArb > CFG.MIN_PROFIT_ARB && (!best.profitable || netArb > best.profit)) {
            best = {
                profitable: true,
    ...
    if (best.profitable)
        console.log(`\n✅ Best triangular: ${best.pathName}, net ${ethers_1.ethers.formatUnits(best.profit, 18)} ARB`);
    else
        console.log("\nNo profitable triangular paths found");
    return best;
}
/* ------------------------------ Multi-token complex (demo) ------------------------------ */
async function findMultiTokenOpportunities(provider) {
    if (!CFG.ENABLE_MULTI_PAIR)
        return { profitable: false };
    ...
    for (const path of complex) {
        ...
        for (let i = 0; i < path.tokens.length - 1; i++) {
            ...
            amt = bestHop;
            gasEst += ethers_1.ethers.parseUnits("0.0002", 18); // rough ARB per hop
        }
        if (!ok)
            continue;
        ...
        console.log(`  Gross(ARB): ${ethers_1.ethers.formatUnits(profitArb, 18)} | Gas est: ${ethers_1.ethers.formatUnits(gasEst, 18)} | Net: ${ethers_1.ethers.formatUnits(net, 18)} ARB`);
        if (net > CFG.MIN_PROFIT_ARB && (!best.profitable || net > best.profit)) {
            best = { profitable: true, strategy: "Multi-Token-Complex", pathName: path.name, profit: net, grossProfit: profitArb, gasAssumed: gasEst, path: path.tokens.join(" -> ") };
        }
    }
    return best;
}
/* ------------------------------ Flash loans (placeholder) ------------------------------ */
async function findFlashLoanOpportunity(_provider) {
    if (!CFG.ENABLE_FLASH_LOANS)
        return { profitable: false };
    console.log("=== SCANNING FLASH LOAN ARBITRAGE ===");
    return { profitable: false };
}
/* ------------------------------ Orchestrator ------------------------------ */
async function runProfitableMEVBot(provider) {
    ...
    // Quoter smoke test
    try {
        const t = ethers_1.ethers.parseUnits("0.001", 18);
        const q = await quoteUniV3(provider, ADDR.ARB, ADDR.WETH, 3000, t);
        if (q === 0n) {
            console.log("Quoter test failed (0)");
            return null;
        }
        console.log(`Quoter ok: ${ethers_1.ethers.formatUnits(t, 18)} ARB → ${ethers_1.ethers.formatUnits(q, 18)} WETH`);
    }
    catch (e) {
        console.log("Quoter test error:", e);
        return null;
    }
    // Show config snapshot
    console.log("\nConfig:");
    console.log(`  Trade size (ARB): ${ethers_1.ethers.formatUnits(CFG.PROBE_NOTIONAL_A, 18)}`);
    console.log(`  Min profit:       ${ethers_1.ethers.formatUnits(CFG.MIN_PROFIT_ARB, 18)} ARB`);
    ...
    const names = [];
    if (CFG.ENABLE_CROSS_DEX) {
        fns.push(() => findCrossDEXOpportunities(provider));
        names.push("Cross-DEX-Multi-Pair");
    }
    if (CFG.ENABLE_TRIANGULAR) {
        fns.push(() => findTriangularOpportunity(provider));
        names.push("Triangular-Multi-Path");
    }
    if (CFG.ENABLE_MULTI_PAIR) {
        fns.push(() => findMultiTokenOpportunities(provider));
        names.push("Multi-Token-Complex");
    }
    if (CFG.ENABLE_FLASH_LOANS) {
        fns.push(() => findFlashLoanOpportunity(provider));
        names.push("Flash-Loan");
    }
    if (!fns.length) {
        console.log("No strategies enabled");
        return null;
    }
        ...
        if (r.status === "fulfilled") {
            ...
            if (val?.profitable && val.profit) {
                if (!best || val.profit > best.profit)
                    best = { strategy: name, profit: val.profit, details: val };
                console.log(`${name}: ${ethers_1.ethers.formatUnits(val.profit, 18)} ARB`);
            }
            else {
    ...
    if (best?.details?.profitable) {
        ...
        console.log(`  Net Profit: ${ethers_1.ethers.formatUnits(best.profit, 18)} ARB`);
        const d = best.details;
        if (d.grossProfit)
            console.log(`  Gross Profit: ${ethers_1.ethers.formatUnits(d.grossProfit, 18)} ARB`);
        if (d.path)
            console.log(`  Path: ${d.path}`);
        if (d.pathName)
            console.log(`  Strategy: ${d.pathName}`);
        if (d.profitBPS)
            console.log(`  Margin: ${d.profitBPS} bps (${(d.profitBPS / 100).toFixed(2)}%)`);
        return best.details;
    }
...
async function startContinuousMonitoring(provider) {
    ...
    let scans = 0, hits = 0, total = 0n, best = null;
    const runScan = async () => {
        scans++;
        console.log(`\n--- Scan #${scans} ---`);
        try {
            ...
            if (opp?.profitable) {
                ...
                if (newBest)
                    best = opp;
                console.log(`HIT! Profit: ${ethers_1.ethers.formatUnits(p, 18)} ARB${newBest ? " (NEW BEST)" : ""}`);
                if (CFG.SAVE_TRADE_HISTORY) {
                    console.log("Record:", JSON.stringify({
                        t: new Date().toISOString(), scan: scans, strategy: opp.strategy, profit: ethers_1.ethers.formatUnits(p, 18)
                    }, null, 2));
                }
            }
            else {
            ...
            console.log(`Stats: ${hits}/${scans} profitable (${rate}%), total ${ethers_1.ethers.formatUnits(total, 18)} ARB`);
        }
        catch (e) {
    ...
    if (typeof process !== "undefined" && process.on) {
        process.on("SIGINT", () => {
            clearInterval(iv);
            const rate = scans ? ((hits / scans) * 100).toFixed(2) : "0.00";
            console.log(`\nFINAL STATS\n  scans: ${scans}\n  hits: ${hits}\n  rate: ${rate}%\n  total: ${ethers_1.ethers.formatUnits(total, 18)} ARB`);
            if (best)
                console.log(`  best: ${ethers_1.ethers.formatUnits(best.profit || 0n, 18)} ARB (${best.strategy})`);
            process.exit(0);
        });
    }
}
