...
async function main() {
  ...
  const usdcN = new ethers.Contract(USDCn, ERC20_ABI, provider);
  ...
  console.log('WETH :', ethers.formatUnits(wethBal,  wethDec));
  console.log('USDC :', ethers.formatUnits(usdcNBal, usdcNDec), '(native)');
  console.log('USDCe:', ethers.formatUnits(usdcEBal, usdcEDec), '(bridged)');
}
