...
type Counters = { daily: number; monthly: number; dayKey: string; monthKey: string; };
const state: Counters = { daily: 0, monthly: 0, dayKey: "", monthKey: "" };
...
function rollover() {
  const d = dayKeyUTC(); const m = monthKeyUTC();
  if (state.dayKey !== d) { state.daily = 0; state.dayKey = d; console.log("[CU] Daily budget reset."); }
  if (state.monthKey !== m) { state.monthly = 0; state.monthKey = m; console.log("[CU] Monthly budget reset."); }
}

function pct(n: number, of: number) { return Math.round((n / of) * 1000) / 10; }

export function getCUStatus() {
  rollover();
  return {
...
export function chargeCU(units: number, p: Priority) {
  rollover();
  // allow EMERGENCY operations to bypass emergency brake slightly
  const s = getCUStatus();
  if (s.daily >= dailyLimit * (emergencyPct / 100) && p < Priority.EMERGENCY) {
...
export const CU_WEIGHTS: Record<string, number> = {
  "eth_blockNumber": 1,
  "eth_getBalance": 2,
