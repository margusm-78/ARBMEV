...
export const WORKING_CONFIG = {
  ...
  // Quoter: support both legacy and current names
...
function resolvedAddrs() {
  return {
    ...
    QUOTER: normAddr("UNI_QUOTER/UNISWAP_V3_QUOTER_V2", WORKING_CONFIG.QUOTER),
...
export async function getWorkingQuote(provider?: ethers.Provider) {
  const p = provider ?? (await getCUProvider());
  ...
  const best = await bestRouteByEdge(p, inARB);
  if (!best) {
 ...
 * NOTE: getLogs is CU-expensive; our CU wrapper charges it heavily. Span & lag are tunable in .env.
 */
...
export function setupWorkingPoolMonitor(provider?: ethers.Provider) {
  ...
  const init = async () => {
    p = provider ?? (await getCUProvider());
    const latestNow = await p.getBlockNumber();
            ...
            if (logs.length > 0) {
              for (const log of logs) {
                try {
                  ...
                  await previewAndMaybeExecute(p);
                } catch (e: any) {
...
export function printUpdatedEnvValues() {
  ...
  console.log("UNI_QUOTER=0x61fFE014bA17989E743c5F6cB21e        # or UNISWAP_V3_QUOTER_V2");
...
export async function runWorkingBot(provider?: ethers.Provider) {
  ...
  try {
    ...
    // 1) Sanity quote
    await getWorkingQuote(p);

    // 2) Start resilient monitor (polling, not filters)
    const cleanup = setupWorkingPoolMonitor(p);
    ...
    console.log("Checked keys: UNI_QUOTER/UNISWAP_V3_QUOTER_V2, TOKEN_ARB, TOKEN_WETH, ARB_WETH_POOL_03, ARB_WETH_POOL_005, CONFIG.router, ROUTERS.swapRouter02");
    process.exit(1);
  }
}
